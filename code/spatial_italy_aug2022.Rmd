...
setwd("/home/kbayliss/extra_code/Italy_background_inlabru")
#source("source/inlabru_forecast_functions.R")
source("/home/kbayliss/extra_code/Italy_background_inlabru/code/inlabru_import_functions.R")
...
Horus_m4 <- read.csv("/home/kbayliss/extra_code/Italy_background_inlabru/input/Horus_GKDec_1960_2020.csv")
...
ItalyCollectionPolygon <- read.delim("/home/kbayliss/extra_code/Italy_background_inlabru/input/ItalyCollectionPolygon.txt", header=FALSE)
...
hest <- sf::st_intersection(st_as_sf(Horus_m4_dec), st_as_sf(It_coll_poly_km))
hm4 <- as(hest, "Spatial")

## No idea why this no longer works, but seems to be a CRS issue :/
#hm4 <- raster::crop(Horus_m4_dec, It_coll_poly_km)
...
```{r}
strainratedata <- read.delim("/home/kbayliss/extra_code/Italy_background_inlabru/input/strainratedata.txt")
...
StrainRateFn_focal3 <- function(xy) {
...
DEM model (NEW!)
```{r}
DEM_merge <- terra::rast("/home/kbayliss/extra_code/Italy_background_inlabru/input/Italy_DEM_merged.TIF")

DEM_min <- -29.16
DEM_max <- 4790.073

DEM_norm_fn <- function(xy) {
  # turn coordinates into SpatialPoints object:
...
IHFC_2021_GHFDB <- read.csv("/home/kbayliss/extra_code/Italy_background_inlabru/input/IHFC_2021_GHFDB.csv", header=TRUE, sep=";")
...
```{r}
Faults3 <- vect("/home/kbayliss/extra_code/Italy_background_inlabru/input/DISS_3.2.1_shp/SHP/CSSPLN321.shp")
sf_faults <- st_as_sf(Faults3)

FaultDistFn= function(xy) {
  # turn coordinates into SpatialPoints object
...
# New hm4 has messed up coord column names :7
hist(FaultDistFn(hm4$coords.x1, hm4$coords.x2))
...
max(FaultDistNormFn(hm4$coords.x1, hm4$coords.x2))
```
Historic (smoothed) seismicity
```{r}
Hseism<- readRDS("/home/kbayliss/extra_code/Italy_background_inlabru/input/CPTI_model_km.RDS")
#Hseism <- model_hist.pred


Hseism$med_norm <- (Hseism$median - min(Hseism$median))/(max(Hseism$median) - min(Hseism$median))
HS_Norm_Fn <-  function(xy){
  # turn coordinates into SpatialPoints object:
  spp = SpatialPoints(data.frame(x = xy[, 1], y = xy[, 2]))
  #spp = SpatialPoints(data.frame(x=x,y=y))
  # attach the appropriate coordinate reference system (CRS)
  proj4string(spp) = CRS(proj4string(Hseism))
...
```
These give the same results now, it's just that at specificallt 7200, 4500 they are both 0
Tectonic zones from ESHM20
```{r}
TectZones <- readOGR("~/data/eshm20-master/input_shapefiles/eshm20_input_c_tecto_zones/eshm20_tecto_winGT_fs017_dm02_results_v12e.shp")
  #readOGR("C://Users/kirst/Documents/GitHub/Italy_background_inlabru/input/eshm20_input_c_tecto_zones/eshm20_tecto_winGT_fs017_dm02_results_v12e.shp")
...
new.extent <- extent(c(5, 20, 35, 49))
par(mfrow=c(3,3))
...
## Plot slip rate
plot(Faults3, "SRmax", axes=FALSE, main="slip rates",  col= brewer.pal(9, "YlGnBu"), ext = new.extent)
lines(it_vect)

## plot tect zones
#TZ_vec <- vect("~/data/eshm20-master/input_shapefiles/eshm20_tecto_winGT_fs017_dm02_results_v12e.shp")
TZ_vec <- vect("~/data/eshm20-master/input_shapefiles/eshm20_input_c_tecto_zones/eshm20_tecto_winGT_fs017_dm02_results_v12e.shp")
...
##
plot(it_vect, axes=FALSE, ext = new.extent)
...
```{r, warning=FALSE}
start_time <- Sys.time()
rfo.mdl= coordinates~ Smooth(main=coordinates, model=spde.model) +  Intercept(1)
rfo.fit = lgcp(rfo.mdl, hm4,  domain = list(coordinates = mesh3), 
                    options = list(E = 50), samplers = It_coll_poly_km)
end_time <- Sys.time()
end_time- start_time
```
...
#tectSR.fit, tectMaghist.fit, tectDEM.fit, slipdist_bzone.fit, slipDistDEM.fit, TectMagDEM.fit, TectMagSR.fit, TectMagSRhist.fit,
deltaIC( SRhistInv.fit, SRhistDEMMa.fit, SRDEMInv.fit, histDEMhf.fit, SRhistDEM.fit, InvHF.fit, hfhist.fit, hfMa.fit, SRMaIFD.fit, SRInv.fit, histInv.fit, SRhistMaSlip.fit, InvDEM.fit, TectSRhistDEM.fit, TectSRhist.fit)
...
```
```{r}
## Rerun to update figure
start_time <- Sys.time()
SRhistMaSlip.mdl= coordinates~ SRN(main=StrainRateFn_focal3(coordinates(.data.)), model="linear") + med_norm(main=HS_Norm_Fn(coordinates(.data.)), model="linear")+ SRMa_norm(main=MaxSlipFn_sp(coordinates(.data.)), model="linear") + Smooth(main=coordinates, model=spde.model) +  Intercept(1)
SRhistMaSlip.fit = lgcp(SRhistMaSlip.mdl, hm4,  domain = list(coordinates = mesh3), 
                        samplers=It_coll_poly_km, 
                        options = list(E = 50))
end_time <- Sys.time()
...
start_time <- Sys.time()
SR.mdl= coordinates~ SRN(main=StrainRateFn_focal3(x, y), model="linear") + Smooth(main=coordinates, model=spde.model) +  Intercept(1)
SR.fit = lgcp(SR.mdl, hm4,  domain = list(coordinates = mesh3), 
                        samplers=It_coll_poly_km, 
                        options = list(E = 50))
end_time <- Sys.time()
end_time- start_time
...
start_time <- Sys.time()
SR.mdl= coordinates~ SRN(StrainRateFn_focal3(coordinates(.data.)), model="linear") + Smooth(main=coordinates, model=spde.model) +  Intercept(1)
SR.fit = lgcp(SR.mdl, hm4,  domain = list(coordinates = mesh3), 
                        options = list(E = 50))
end_time <- Sys.time()
end_time- start_time
...
start_time <- Sys.time()
SR.mdl= coordinates~ SRN(eval_spatial(test4), model="linear") + Smooth(main=coordinates, model=spde.model) +  Intercept(1)
SR.fit = lgcp(SR.mdl, hm4,  domain = list(coordinates = mesh3), 
                        options = list(E = 50))
end_time <- Sys.time()
end_time- start_time
...
SRhistDEM.pr <- predict(SRhistDEM.fit, pixels(mesh3, mask=It_coll_poly_km), ~(SRN + hist + DEM + Smooth + Intercept))

InvDEMMa.pr <- predict(InvDEMMa.fit, pixels(mesh3, mask=It_coll_poly_km), ~(FDN+ DEM + Ma + Smooth + Intercept))
```

```{r}
SRhistMa.pr <- predict(SRhistMaSlip.fit, pixels(mesh3, mask=It_coll_poly_km), ~(SRN + hist + Ma + Smooth + Intercept))
histInv.pr <- predict(histInv.fit, pixels(mesh3, mask=It_coll_poly_km), ~(hist + FDN + Smooth + Intercept))
SRMaIFD.pr <- predict(SRMaIFD.fit, pixels(mesh3, mask=It_coll_poly_km), ~(SRN + Ma + FDN + Smooth + Intercept))
SRInv.pr <- predict(SRInv.fit, pixels(mesh3, mask=It_coll_poly_km), ~(SRN + FDN + Smooth + Intercept))
...
lprange <- range(SRhistMa.pr$mean, SRMaIFD.pr$mean, SRInv.pr$mean,  histInv.pr$mean, InvDEMMa.pr$mean, SRhistDEM.pr$mean )
csc <- scale_fill_gradientn(colours = brewer.pal(9, "YlOrRd"), limits = lprange)

SRhistMaplt <- ggplot() + gg(SRhistMa.pr) + ggtitle("SR + hist + maxslip") + csc  + gg(it_coast, alpha = 0)
histInvplt <- ggplot() + gg(histInv.pr) + ggtitle("hist + IFD")  + csc  + gg(it_coast, alpha = 0)
SRMaIFDplt <- ggplot() + gg(SRMaIFD.pr) + ggtitle("SR + maxslip + IFD") + csc + gg(it_coast, alpha = 0)
SRInvplt <- ggplot() + gg(SRInv.pr) + ggtitle("SR + IFD") + csc  + gg(it_coast, alpha = 0)
SRhistDEMplt <- ggplot() + gg(SRhistDEM.pr) + ggtitle("SR + hist + DEM") + csc  + gg(it_coast, alpha = 0)
InvDEMMaplt <- ggplot() + gg(InvDEMMa.pr) + ggtitle("IFD + DEM + maxslip") + csc  + gg(it_coast, alpha = 0)



multiplot(SRhistMaplt, histInvplt, SRMaIFDplt, SRInvplt, SRhistDEMplt, InvDEMMaplt,  cols=2)
```

```{r}
pdf('some_inlabru_outputs_itmap_test2.pdf')
multiplot(SRhistMaplt, histInvplt, SRMaIFDplt, SRInvplt, SRhistDEMplt, InvDEMMaplt,cols=2)
dev.off()
```
```{r}
pdf('three_running_inlabru_outputs.pdf')
multiplot(SRhistMaplt, histInvplt,  SRInvplt, cols=1)
dev.off()
```
...
# Well this ran with covariate values but it gives me an obscene number of events...
Pred_Num_SRInv
...
ips <- ipoints(It_coll_poly_km, mesh3)
start_time <- Sys.time()
Pred_Num_SRhistMA <- predict(SRhistMaSlip.fit, ips, ~ sum(weight * exp(SRN + hist + Ma + Smooth + Intercept)))
end_time <- Sys.time()
end_time- start_time

# These are all evaluationg e30 events, which is clearly not right. 
```
```{r, warning=FALSE}
